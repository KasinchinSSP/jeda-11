...
export default function Hero({ eyebrow, title, subtitle, video, overlay }: HeroProps) {
  const { landscape, portrait, strategy } = video;

  const landscapeVisibility =
    strategy === "portrait" ? "hidden" : strategy === "landscape" ? "block" : "hidden landscape:block";
  const portraitVisibility =
    strategy === "landscape" ? "hidden" : strategy === "portrait" ? "block" : "hidden portrait:block";
  ...
  return (
    <section className="relative isolate flex min-h-screen items-center overflow-hidden">
      <div className="absolute inset-0 -z-10">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-slate-950/90 via-slate-950/40 to-slate-950" aria-hidden />
      </div>
